...
memefs_superblock_t *getSuperBlockMain(struct stat *statbuf) {
...
void getDirectories(memefs_superblock_t *sa) {
    //Get the Directory
...
void getMainFat(memefs_superblock_t *sa) {
    //Get the MainFat
...
void writeMap(struct stat statbuf){ 
...
int closeMMap(struct stat statbuf) {
 ...
 int myfs_getattr(const char *path, struct stat *st){
    ...
    struct  stat *buf  = openFile();
    memefs_superblock_t *b = getSuperBlockMain(buf);
    ...
    st->st_blocks = b->num_user_blocks;
